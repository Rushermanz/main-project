...
class MovementRecorder:
    ...
    def save_to_file(self, filename="usa_player_run.json"):
...
class Track(pygame.sprite.Sprite):
    def __init__(self, pos, group):
        ...
        self.image = scale_image(pygame.image.load('assets/usa.png').convert_alpha(), 2.9)
...
player = Red_car((2190, 5800), camera_group)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_s:
            recorder.save_to_file("usa_player_run.json")
